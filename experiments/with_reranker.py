"""
Experiment: full pipeline — hybrid retrieval + cross-encoder reranker.

This is the production configuration.  The cross-encoder jointly encodes
(query, chunk) pairs, catching relevance signals that cosine similarity misses.

Usage:
    python experiments/with_reranker.py
    python experiments/with_reranker.py --k 9
"""
...
def run(k: int = 9) -> None:
    print(f"=== Full pipeline experiment (hybrid + reranker, k={k}) ===\n")
    ...
    out = RESULTS_DIR / f"hybrid_rerank_k{k}.json"
